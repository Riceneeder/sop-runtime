...
import {validateRuntimeValue} from '../src/index.js';
...
describe('validateRuntimeValue', () => {
  ...
  test('supports enum and const with deep value comparison', () => {
    ...
    expect(failResult.diagnostics).toHaveLength(2);
  });
  ...
  test('reports malformed supported schema keyword values', () => {
    ...
    expect(result.ok).toBe(false);
    expect(result.diagnostics).toEqual(expect.arrayContaining([
      expect.objectContaining({'code': 'schema_type', 'path': '$schema.properties'}),
      expect.objectContaining({'code': 'schema_type', 'path': '$schema.required'}),
      expect.objectContaining({'code': 'schema_type', 'path': '$schema.minimum'}),
    ]));
    expect(result.diagnostics).toHaveLength(3);
  });

  test('separates schema keyword paths from runtime value paths', () => {
    const result = validateRuntimeValue({
      'path': 'payload.items',
      'schema': {
        'type': 'object',
    ...
    expect(result.ok).toBe(false);
    expect(result.diagnostics).toEqual(expect.arrayContaining([
      expect.objectContaining({'code': 'schema_type', 'path': '$schema.payload.items.minProperties'}),
      expect.objectContaining({'code': 'schema_required', 'path': 'payload.items.name'}),
    ]));
  ...
  test('reports malformed nested and array schema keywords', () => {
    const result = validateRuntimeValue({
      'schema': {
        // 123 covers non-string items; "unknown" covers unsupported string items.
        'type': ['object', 123, 'unknown'],
        'properties': {
          'items': {
            'type': 'array',
            'items': ['not-object'],
    ...
    expect(result.ok).toBe(false);
    expect(result.diagnostics).toEqual(expect.arrayContaining([
      expect.objectContaining({'code': 'schema_type', 'message': 'Expected string.', 'path': '$schema.type.1'}),
      expect.objectContaining({'code': 'schema_enum', 'path': '$schema.type.2'}),
      expect.objectContaining({'code': 'schema_type', 'path': '$schema.properties.items.items.0'}),
      expect.objectContaining({'code': 'schema_minimum', 'path': '$schema.properties.items.minItems'}),
      expect.objectContaining({'code': 'schema_type', 'path': '$schema.properties.extra'}),
      expect.objectContaining({'code': 'schema_type', 'path': '$schema.additionalProperties'}),
      expect.objectContaining({'code': 'schema_type', 'path': '$schema.minLength'}),
    ]));
