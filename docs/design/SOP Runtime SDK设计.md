...
`defineSop` 的职责是提供类型约束和可选默认值填充。真正准入仍由 `validateDefinition` 完成。
...
1. 调整 `definition` 中 executor DSL，从固定 `sandbox_tool` / `sandbox_script` / `sandbox_model` 改为通用 `kind + name + config`。
2. 更新 JSON Schema、validator、README 和测试。
3. 在 `RunState` 中加入 `paused` phase 与 pause metadata。
