...
This Bun workspace is organized by package responsibility under `packages/`. The active packages in this worktree are `definition`, `validator`, `core`, and `runtime`; public entrypoints live at `packages/*/src/index.ts`, tests live in sibling `packages/*/test/` directories, and build artifacts are emitted to `packages/*/dist`. Keep test files named `*.test.ts`, for example `packages/core/test/index.test.ts`. Repo-level docs live in `docs/design/`, and the local TypeScript style reference is mirrored in `references/google_typescript_styleguide/`.
...
- `bun test packages/definition/test/index.test.ts`: run one test file.
- `bun test packages/core/test --test-name-pattern "createRun"`: run a focused test subset.
...
## Testing Guidelines
Tests use `bun:test` and live in each package’s sibling `test/` directory. Add or update tests with every behavior change, especially when touching shared contracts in `definition`, `validator`, `core`, or `runtime`. There is no published coverage gate, so contributors are expected to run `bun run check` before opening a PR.
